import Link from "next/link";
import { usePathname } from "next/navigation";

const tabs = [
  {
    name: "Home",
    href: "/",
    icon: (active: boolean) => (
      <svg viewBox="0 0 24 24" className={`h-6 w-6 ${active ? "text-black" : "text-gray-500"}`}>
        <path
          fill="currentColor"
          d="M23 9v2h-2v7a3 3 0 01-3 3h-4v-6h-4v6H6a3 3 0 01-3-3v-7H1V9l11-7 5 3.18V2h3v5.09z"
        />
      </svg>
    ),
  },
  {
    name: "Network",
    href: "/network",
    icon: (active: boolean) => (
      <svg viewBox="0 0 24 24" className={`h-6 w-6 ${active ? "text-black" : "text-gray-500"}`}>
        <path
          fill="currentColor"
          d="M12 16v6H3v-6a3 3 0 013-3h3a3 3 0 013 3zm5.5-3c1.94 0 3.5 1.56 3.5 3.5S19.44 20 17.5 20 14 18.44 14 16.5s1.56-3.5 3.5-3.5zm3.5-6v2H3V7h18zm-6-6v2H3V1h12z"
        />
      </svg>
    ),
  },
  {
    name: "Jobs",
    href: "/jobs",
    icon: (active: boolean) => (
      <svg viewBox="0 0 24 24" className={`h-6 w-6 ${active ? "text-black" : "text-gray-500"}`}>
        <path
          fill="currentColor"
          d="M17 6V5a3 3 0 00-3-3h-4a3 3 0 00-3 3v1H2v4a3 3 0 003 3h14a3 3 0 003-3V6h-5zm-2 0h-6V5a1 1 0 011-1h4a1 1 0 011 1v1zm7 8h-2v2h-2v2h2v2h2v-2h2v-2h-2v-2z"
        />
      </svg>
    ),
  },
  {
    name: "Notifications",
    href: "/notifications",
    icon: (active: boolean) => (
      <svg viewBox="0 0 24 24" className={`h-6 w-6 ${active ? "text-black" : "text-gray-500"}`}>
        <path
          fill="currentColor"
          d="M13.7 19C13.9 19.3 14 19.6 14 20C14 21.1 13.1 22 12 22C10.9 22 10 21.1 10 20C10 19.6 10.1 19.3 10.3 19H2V18C2 17 2.4 16.1 3.2 15.2L4.2 14H19.9L20.9 15.2C21.7 16.2 22.1 17.1 22.1 18V19H13.7ZM18.2 7.4C17.8 4.3 15.1 2 12 2C8.9 2 6.2 4.3 5.8 7.4L5 13H19L18.2 7.4Z"
        />
      </svg>
    ),
  },
  {
    name: "Messages",
    href: "/messages",
    icon: (active: boolean) => (
      <svg viewBox="0 0 24 24" className={`h-6 w-6 ${active ? "text-black" : "text-gray-500"}`}>
        <path
          fill="currentColor"
          d="M16 3H8C6.14348 3 4.36301 3.77847 3.05025 5.16416C1.7375 6.54984 1 8.42923 1 10.3889C1 12.3485 1.7375 14.2279 3.05025 15.6136C4.36301 16.9993 6.14348 17.7778 8 17.7778H12V22L20.16 16.3106C21.0512 15.639 21.7751 14.7495 22.2697 13.7183C22.7643 12.687 23.0148 11.5446 23 10.3889C23 8.42923 22.2625 6.54984 20.9497 5.16416C19.637 3.77847 17.8565 3 16 3ZM8 11.7083C7.75277 11.7083 7.5111 11.631 7.30554 11.486C7.09998 11.341 6.93976 11.1349 6.84515 10.8938C6.75054 10.6527 6.72579 10.3874 6.77402 10.1315C6.82225 9.87553 6.9413 9.64043 7.11612 9.4559C7.29093 9.27137 7.51366 9.14571 7.75614 9.0948C7.99861 9.04389 8.24995 9.07002 8.47835 9.16988C8.70676 9.26975 8.90199 9.43886 9.03934 9.65585C9.17669 9.87283 9.25 10.1279 9.25 10.3889C9.25 10.7388 9.1183 11.0744 8.88388 11.3219C8.64946 11.5693 8.33152 11.7083 8 11.7083ZM12 11.7083C11.7528 11.7083 11.5111 11.631 11.3055 11.486C11.1 11.341 10.9398 11.1349 10.8452 10.8938C10.7505 10.6527 10.7258 10.3874 10.774 10.1315C10.8222 9.87553 10.9413 9.64043 11.1161 9.4559C11.2909 9.27137 11.5137 9.14571 11.7561 9.0948C11.9986 9.04389 12.25 9.07002 12.4784 9.16988C12.7068 9.26975 12.902 9.43886 13.0393 9.65585C13.1767 9.87283 13.25 10.1279 13.25 10.3889C13.25 10.7388 13.1183 11.0744 12.8839 11.3219C12.6495 11.5693 12.3315 11.7083 12 11.7083ZM16 11.7083C15.7528 11.7083 15.5111 11.631 15.3055 11.486C15.1 11.341 14.9398 11.1349 14.8452 10.8938C14.7505 10.6527 14.7258 10.3874 14.774 10.1315C14.8222 9.87553 14.9413 9.64043 15.1161 9.4559C15.2909 9.27137 15.5137 9.14571 15.7561 9.0948C15.9986 9.04389 16.25 9.07002 16.4784 9.16988C16.7068 9.26975 16.902 9.43886 17.0393 9.65585C17.1767 9.87283 17.25 10.1279 17.25 10.3889C17.25 10.7388 17.1183 11.0744 16.8839 11.3219C16.6495 11.5693 16.3315 11.7083 16 11.7083Z"
        />
      </svg>
    ),
  },
];

export function MobileNav() {
  const pathname = usePathname();

  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200">
      <div className="flex justify-around items-center h-14">
        {tabs.map((tab) => {
          const isActive = pathname === tab.href;
          return (
            <Link
              key={tab.name}
              href={tab.href}
              className="flex flex-col items-center justify-center w-full h-full"
            >
              {tab.icon(isActive)}
              <span className={`text-xs mt-1 ${isActive ? "text-black" : "text-gray-500"}`}>
                {tab.name}
              </span>
            </Link>
          );
        })}
      </div>
    </nav>
  );
} 